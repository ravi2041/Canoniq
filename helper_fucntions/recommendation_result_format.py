...
def _parse_input(raw):
    """Accept list[dict], dict, JSON string, or Python-literal string."""
...
def _dir_from_text(s: str) -> str:
    s = s.lower()
    if "↑" in s or "up" in s: return "up"
    if "↓" in s or "down" in s or "reduce" in s: return "down"
    return "unknown"
...
def _dimension_from_text(s: str) -> str:
    s = s.lower()
    for k in ("cpa","cvr","ctr","cpc","roas","revenue","engagement","cost"):
        if k in s: return k.upper()
    return "ENGAGEMENT"

def _parse_action(a):
    """Return action as dict. If already a dict, return as-is."""
    if isinstance(a, dict):
        return a
    t = _clean(str(a)).lower()
    if "reallocat" in t: verb = "reallocate_budget"
    elif "increase spend" in t or "scale" in t: verb = "scale_budget"
    elif "pause" in t: verb = "pause_entity"
    elif "audit" in t: verb = "audit_tracking"
    elif "enable conversion" in t or "enable conversions" in t: verb = "enable_conversion_tracking"
    elif "standardize" in t and "campaign" in t: verb = "standardize_naming"
    elif "test" in t or "a/b" in t or "ab test" in t: verb = "ab_test_creatives"
    else: verb = "review"
    m_pct = _PCT.search(t)
...
def _has_metric(collected, metric_label):
    """Check if a metric with same label already exists (avoid duplicates)."""
    for m in collected:
        if m.get("metric") == metric_label:
            return True
    return False

def _add_metric(collected, metric_label, formatted_value):
    if formatted_value is None or formatted_value == "":
        return
    if not _has_metric(collected, metric_label):
        collected.append({"metric": metric_label, "formatted": formatted_value})

def _extract_first_pct(text):
    m = _PCT.search(text)
    return f"{m.group(1)}%" if m else None

def _extract_first_money(text):
...
def _normalize_metrics(metrics_list, why_text):
    ...
    Returns a deduped list with detected CTR/CPC/CVR/CPA extracted from both metrics and 'why'.
    """
    out = []
    lst = metrics_list or []

    # Keep original items as 'snippet' (string) or pass-through if already structured
    snippet_parts = []
    for item in lst:
        if isinstance(item, dict) and "metric" in item and "formatted" in item:
            # keep as-is but dedupe by metric label
            _add_metric(out, item["metric"], _clean(item["formatted"]))
            snippet_parts.append(str(item.get("formatted", "")))
        else:
            # treat as raw snippet
            txt = _clean(str(item))
            if txt:
    ...
    # Detect explicit metrics if not already present
    # CTR
    m_ctr = re.search(r'ctr[^0-9%]*' + _PCT.pattern, blob, re.IGNORECASE)
    if m_ctr:
        _add_metric(out, "CTR", f"{m_ctr.group(1)}%")
    # CPC
    m_cpc = re.search(r'cpc[^0-9$€£]*' + _MONEY.pattern, blob, re.IGNORECASE)
    if m_cpc:
        _add_metric(out, "CPC", f"${m_cpc.group(1).replace(',', '').strip()}")
    # CVR
    m_cvr = re.search(r'cvr[^0-9%]*' + _PCT.pattern, blob, re.IGNORECASE)
    if m_cvr:
        _add_metric(out, "CVR", f"{m_cvr.group(1)}%")
    # CPA
    m_cpa = re.search(r'cpa[^0-9$€£]*' + _MONEY.pattern, blob, re.IGNORECASE)
    if m_cpa:
        _add_metric(out, "CPA", f"${m_cpa.group(1).replace(',', '').strip()}")
